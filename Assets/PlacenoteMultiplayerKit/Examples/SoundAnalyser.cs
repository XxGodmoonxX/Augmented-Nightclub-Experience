...
public class SoundAnalyser : MonoBehaviour {
	...
	void Start () {
		...
		for (int i = 0; i < soundObject.GetComponent<sound>().spectrumNum.Length; i++) {
			...
			Vector3 posRight = new Vector3 (3f, 0, 0.05f * i); //真ん中から均等に上下に 右側の波
			linesRight[i] = Instantiate(Cube, posRight, Quaternion.identity) as GameObject;

			Vector3 posLeft = new Vector3 (-3f, 0, 0.05f * i); //真ん中から均等に上下に 左側の波
	...
	void Update () {
		for (int i = 0; i < soundObject.GetComponent<sound>().spectrumNum.Length; i++) {
			...
			//各周波数の大きさをCubeの高さに反映
			linesRight[i].transform.localScale = new Vector3(0.05f, 0.05f * spectrum[i] * 500, 0.05f);
			linesLeft[i].transform.localScale = new Vector3(0.05f, 0.05f * spectrum[i] * 500, 0.05f);
		}
	}
}
